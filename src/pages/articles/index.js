...
const Index = () => {
    ...
    const { data: campaignCategories } = useSWR(`/api/article-categories`, () => axios.get(`/api/article-categories`).then((res) => {
        ...
        <div>
            ...
            <div className='bg-white py-10'>
                <Container className={'flex flex-col items-center '}>
                    <h3 className='text-2xl font-semibold  text-dark-500'>Pilihan Kategori Artikel</h3>
            ...
            <Container>
                <div className='grid lg:grid-cols-3 gap-6 my-8'>
                    {data?.data.map((item, index) => (
                        <ArticleItem mutate={mutate} data={item} key={index} />
                    ))}
                </div>
            </Container>
