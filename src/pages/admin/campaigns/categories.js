...
const Categories = () => {
    ...
    const { data, mutate } = useSWR(`/api/campaign-categories`, () => axios.get('/api/campaign-categories').then(res => {
        return res.data.data
    }))
    const handleAdd = async (values, actions) => {
        setIsLoading(true)
        await axios.post('/api/campaign-categories', {
            ...values
        }).then(res => {
    ...
    return (
        <div>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-bold text-dark-800 mb-4'>Daftar Kategori Program Wakaf</h1>
                <div>
            ...
            <div className='space-y-4'>
                {data?.map((item, index) => (
                    <CategoryItem type='campaign' mutate={mutate} data={item} key={index} />
                ))}
            </div>
        </div>
    )
